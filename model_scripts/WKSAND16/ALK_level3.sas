...
*if aar le 2005 then delete; *Changed for WKSANDEEL 2022;
*if aar gt 2005 then delete;
run;

proc sort data=aglg5;
by geartype aar month  intsq area PP_ar_tx scm;
run;
...
by geartype aar month intsq area PP_ar_tx  age;
run;
...
by age  geartype aar month intsq area PP_ar_tx maxscm minscm;
run;
...
by age geartype aar month intsq area PP_ar_tx;
run;
...
*data aglg13d; *Changed for WKSANDEEL 2022;
*set out.alk_level3;
*if aar ge 2005 then delete;
*run;

data out.alk_level3;
set aglg13c;* aglg13d; *Changed for WKSANDEEL 2022;
...
pi=n/s;
level=3;
keep age geartype aar month intsq area PP_ar_tx maxp minp prange 
...
/*
proc sort data=out.alk_level3 out=t1;
by  age geartype aar  intsq PP_ar_tx month scm;
run;
...
by age geartype aar intsq PP_ar_tx month;

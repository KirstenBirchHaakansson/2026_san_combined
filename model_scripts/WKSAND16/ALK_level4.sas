...
by geartype aar month PP_ar_tx scm;
run;
...
by geartype aar month PP_ar_tx  age;
run;
...
by age  geartype aar month PP_ar_tx maxscm minscm;
run;
...
by age geartype aar month PP_ar_tx;
run;
...
pi=n/s;
level=4;
keep age geartype aar month PP_ar_tx maxp minp prange 
...
proc sort data=out.alk_level4 out=t1;
by  age geartype aar PP_ar_tx month scm;
run;
...
by age geartype aar PP_ar_tx month;

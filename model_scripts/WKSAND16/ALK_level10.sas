...
by age  geartype aar  maxscm minscm;
run;
...
var minp maxp prange;
by age geartype aar  ;
output out=aglg13b (drop=_type_ _freq_) max()=;
run;
...
data out.alk_level10;
set aglg13c;* aglg13d;
*if maxp gt minp and minp gt 0.00001 then delete;
*if age=0 and maxp gt 0.9999 then delete;
*if prange gt 0.5 then delete;
pi=n/s;
level=10;
keep age geartype aar  maxp minp prange 
...
proc sort data=out.alk_level10 out=t1;
by  age geartype aar  scm;
run;
